...
**s29 ROUND 3 PUSHED LIVE 2026-09-08 (`7860c82d..5a300515`, fast-forward, merge intact on origin, wire-verified: live index.html sha == HEAD blob after 2 polls, per-owner Permission code on the wire).** Earlier text: **s29 ROUND 3 MERGED ON MAIN 2026-09-08 — Track G `s29/dt-permission-per-owner` (`7e5a569a`, merge `5d6c364c`) — pushed on "push" 2026-09-08.** Final main: index 8/0; **4098 passed, 0 failed across 51 suites** (NEW PIN; test-deed-transfer 382→412). Director re-ran the track tree and merged main; byte-identical. RULING (verbatim): "Permission of use would only be one per." → asked: one Permission of Use per owner. SHIPPED: with 2+ living signers the packet carries one filled Permission page per living owner, contiguous where the single page sat (today's order kept: Heirs before Permission), each with that owner's name alone in the signer boxes, own address if typed else shared, own phone; the template's single signature line, nothing drawn (Track F's side-by-side split on that page removed); copies via dtBuildOnePacket → copyPages → re-registered under " po2"/" po3" (Track A's pattern); one signer or heir-affiant case = pristine page. One help-text clause corrected (co-owner address now DOES print, on their Permission copy). OPEN: "the property that belonged to … now deceased" on the Permission fills the deceased owner and falls back to the FIRST owner when nobody is flagged deceased — pre-existing s27 semantics, flagged to the operator; Acrobat look at a 2-signer packet (renamed copied fields); latent crash in a Track D/F split helper on an empty slot array, reachable only if pageCache.invalidate() is removed. Worktree s29e removed; renders scratch/s29-g-renders.

**s29 ROUND 2 PUSHED LIVE 2026-09-08 (`d08e37d4..5e4d22cf`, fast-forward, both --no-ff merges intact on origin, wire-verified: live index.html sha == HEAD blob after 2 polls, dtSigners on the wire ×4).** Earlier text: **s29 ROUND 2 MERGED ON MAIN 2026-09-08 — Track E `s29/dt-co-address` (`38de7b25`, merge `5f9ffc09`) + Track F `s29/dt-sign-room` (`12cafff7`, merge `bbbb8e46`) — pushed on "push" 2026-09-08.** Final main: index 8/0; **4068 passed, 0 failed across 51 suites** (NEW PIN; test-deed-transfer 275→317→382). Director re-ran both trees and merged main; byte-identical. OPERATOR RULINGS 2026-09-08 (verbatim in TRACK-E.md / TRACK-F.md): collect a co-owner's own address on the site (packet unchanged, one address box); on the live Release "three lines… still not enough room for each person to sign", Loss "not enough room"; "list the current owners as the surviving heirs at law"; two deceased owners → "One affidavit naming both"; who signs when one has died → living owners only. SHIPPED: co-owner rows carry street/city/state/zip (blank = same as the first owner), saved, not printed; Release stack pitch adaptive (48/42pt notary/plain at 2 signers, ~38/32pt clear); Loss and Permission signers SIDE BY SIDE across the text width (214.76pt / 131pt a line, 51/54pt clear — Permission stops at its Date rule); erases 0.5pt proud, 600dpi-scanned white; on-rule widgets 2_2, 1_3/2_3, 1_4/2_4 removed at 2+ signers (the Acrobat highlight boxes); Deceased tick + date of death per owner row drives the situation toggle; `dtSigners()` = living owners, name boxes still "A & B"; one living signer = pristine template; Heirs decedent/date lines join all deceased, typed override wins; "List current owners as heirs" button (living owners, own address if typed, no duplicates, cap 5). Track decisions accepted: columns stay inside the page margins (brief's ≥240pt was impossible), NAME NUMERO1/2 are the filled recital not the rule widgets. OPEN: three signers on the Permission (3 × ~80pt lines); the DocuSign field map now sees fewer widgets at 2+ signers. Worktrees s29c removed, s29d to remove; renders scratch/s29-e-renders, s29-f-renders.

**s29 PUSHED LIVE 2026-09-07 (`a867599c..4a2dbc43`), wire-verified: live index.html sha == HEAD blob after 2 polls of deploy lag, dtDrawSignatureSplit on the wire ×9.** SHA CORRECTION: `git pull --rebase` before the push LINEARIZED the two --no-ff merges, so the SHAs cited below are dead labels — merge `4fef98ea`→(gone; track commits `a93f8c33`,`4a043e14` sit directly on main), ops `47730312`→`58d94ce6`, revert `7bd89821`→`570db7ad`, Track B `9e95f80d`→`195c5b03`, Track D `06995900`→`abdf4ba3`, merge `1148822c`→(gone), ops `c11e5f90`→`4a2dbc43`. Trees identical; only history shape changed. SCAR (now in DIRECTOR_GUIDELINES): when main is not behind origin, do NOT pull --rebase before pushing; if it is behind, use `git pull --rebase=merges` so --no-ff merges survive. Cleanup: s29b worktree deregistered but its folder is LOCKED by a dead process handle (s23d/s26a pattern) — delete C:UsersMarticew-quote-tool-s29b after reboot; track branches deleted; renders in scratch/s29-b-renders + s29-d-renders.

**s29 FINAL — MERGED ON MAIN 2026-09-07 (`1148822c`, Track B `9e95f80d` + Track D `06995900` on `s29/dt-joint`), PUSH AUTHORIZED ("push it when the track is done") — see the push line below.** Final main: index 8/0; **3961 passed, 0 failed across 51 suites** (NEW PIN; worktree read 3961 too on this branch; reconciles 3884 + 77 net test-deed-transfer 198→275). Director re-ran check + suite on the track tree and confirmed merged main byte-identical. THE RULING, after three director misreads (per-copy assembly, name-only rows, side-by-side columns): "it's important we give somewhere for both of them to sign but if there's on addresss on the form and not both thats ok as long as it reads as purcsher & co-purchaser" / "separate lines so they don't have to sign so small". SHIPPED: ONE packet (the s27 page set); co-owner rows (cap 3) collect name+phone+email and save on the dt record, printing only where a box exists (single boxes = primary's, shared address); every name box reads "A & B" (`dtOwnerNames()`) — Release grantor, Loss affiant default, Permission signer default, cover row, statement; Heirs decedent default stays the primary alone; STACKED full-width signature lines, one per co-owner with printed name, on Release p2/p3, Loss p4/p5, Permission p7/p8 (`DT_SPLITS` geometry per page, `layout: stack|columns`, rule erased and the stack drawn in the blank band above where a caption hugs the rule); p9 Statement green box stays side-by-side halves (operator named it; 24pt band); signer guard: a typed Loss affiant or the heir affiant on Permission (owner deceased) keeps a single line. Track A's per-copy assembly (`4fef98ea`) is history only. SCARS: (1) director misread the operator three times — when he says "places to sign" he means lines on the ONE form, ask before designing an assembly; (2) a stopped track leaves a dev-server + uncommitted edits — kill by pid, save the diff, checkout; (3) Track B's throwaway script mis-escaped a route filter and the real Firebase SDK signed in against production auth (failed auth/invalid-credential, nothing read/written) — brief tracks to check route filters before any Playwright one-liner; (4) pdf-lib page cache + MediaBox scars from Track A still apply. OPEN: three co-owners on Permission of Use is tight (18pt pitch); renders scratch/s29-b-renders + s29-d-renders; worktree s29b to remove.

**s29 CLOSED ON MAIN 2026-09-07 — Track A `s29/dt-co-owners` MERGED --no-ff (`4fef98ea`; track `a93f8c33` + amendment `4a043e14`). NOT PUSHED — no pre-authorization.** Final main: index 8/0; **3888 passed, 0 failed across 51 suites** (NEW PIN; worktree read 3886, the documented −2; reconciles 3801 + 87 test-deed-transfer 113→200). Director re-ran check + suite on the track tree (3886/51) and on merged main; merged tree byte-identical to the branch. SHIPPED: Deed Transfer current-owner side is a list (primary + up to 2 co-owners, NAME ONLY per operator amendment; address/phone/email stay the primary's single block); every signed document (Release, Loss, Heirs, Permission) gets one filled copy per co-owner in owner order — packet = p1, [docs×A], [docs×B], …, p9, p10; copies 2..N have AcroForm fields re-registered under a " co2"/" co3" suffix (same-named fields would share one value); p9 Statement green box split into N signature columns with each printed name, single address block; cover row joins names with " & "; `coOwners` on the dt record, legacy records restore as one owner. SCARS (promote to every pdf-lib page-copy brief): (1) pdf-lib removePage() never invalidates its page cache — copyPages()/getPage() after a prune hand back the ORIGINAL pages with half-pruned widgets and dangling refs; call doc.pageCache.invalidate() after the prune. (2) This template's MediaBox is [-11.96 11.99 600.04 1019.99], not [0 0 612 1008] — draw via the MediaBox origin, never raw MuPDF coordinates; caught by looking, not counting. (3) A track missed AMENDMENT.md because its brief listed files, not the directory — amendments go INTO the brief, and briefs say "read the whole sprint directory". (4) Track left two dev-servers on 3737/3747 the harness would not let it kill — director killed them before grading. OPEN WITH THE OPERATOR: push word; a co-owner's Release/Permission copy prints a BLANK phone box (no phone collected for a co-owner; the primary's would be wrong under another signature) — confirm or rule; when decedent/heir-affiant are typed explicitly, owner B's Heirs and Permission copies are identical to owner A's (the "all of them" ruling as specified — confirm that is wanted); open one multi-co-owner packet in Acrobat (new page-copy assembly path, renamed fields) before first real use; the " co2" field names are what DocuSign field-mapping would see. Renders: scratch/s29-a-renders (34 files); report: sprints/sprint-29/TRACK-A-REPORT.md. Worktree s29a removed clean.

**Current sprint: sprint-29 — OPENED 2026-09-07** (director: Fable session; track A `s29/dt-co-owners`, Opus, worktree bw-quote-tool-s29a from `a867599c`). Deed Transfer: multiple current co-owners — one copy of Release/Loss/Heirs/Permission per co-owner, p9 Statement current-owner box split into N signature columns with one shared address, new owner stays single, cover row joins names. Operator rulings 2026-09-07 in `sprints/sprint-29/SPRINT.md`. Still open with the operator from s28: FH compare with no payment estimate; monthly promo still discounts an already-owned space.
