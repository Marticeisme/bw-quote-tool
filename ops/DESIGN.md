...
| Assertion suites | `npm test` | `4098 passed, 0 failed across 51 suites` — s29 round 3 (Windows): test-deed-transfer 113→412 (joint names, room to sign, co-owner address, living owners sign, one Permission per owner); the interim per-copy build read 3888 on main / 3886 in a worktree (the documented −2). s28 Windows worktree read was 3799/51 (container 3772/19 was proxy/font reds). s25 operator round: 3336/47 =  3063 + 273 test-guide-nav (hub-only sidebar gate; was 421 site-wide). s23 history: 2854 + 77 Track A + 111 Track B + 21 Track D; earlier: 2854 + 77 Track A (test-ric-line-qty 26 + test-xlsx-print-path 51) + 111 Track B (test-multi-contact-link 59 + test-combined-totals 50 + 2 net reworked asserts) + 21 Track D (test-pcm-catalog 388→409). In a WORKTREE expect −2 (test-contact-csv's wmp-map cross-check skips where wmp-cemetery-map/ is absent) |
