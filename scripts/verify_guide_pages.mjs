...
const PER_GUIDE_CAPS = new Map([
  ...
  // FOUR, which is the ceiling the brief set, and it is a deliberate ceiling rather than
  // the shared six. This guide carries the region graphic (a half page on its own) and the
  // operator's mid-sprint ruling that it must argue WHY a family wants the plan before it
  // describes the plan, so it is longer than the two-page Tier-1 cut and shorter than a
  // property guide. It is emailed on its own as a campaign piece, which is exactly the
  // document you do not want quietly growing past what somebody will read.
  ['Travel Plan by Inman.pdf', 4],
]);
