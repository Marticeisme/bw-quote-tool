...
const FEES = {
  ...
  // The SHEET and data/prices.json now AGREE at $875 (sprint-09 Track S, 2026-07-31 —
  // the operator ruled "875, sheet wins" and the source behind build-prices.py was
  // changed and both copies rebuilt). Kept sourced from SHEET so the sheet stays the
  // stated authority for this page; the two are asserted equal at the bottom of the
  // file, so they can no longer drift apart silently.
...
// ── the sheet and data/prices.json must AGREE ────────────────────────────────
// This block used to only NOTE a disagreement, because prices.json is generated by the
// map's build-prices.py (DESIGN §7) and could not be corrected from this repo — while
// the guide printed the sheet's $875 and the tool billed $985. Sprint-09 Track S closed
// that: the operator ruled "875 — sheet wins", the source build-prices.py reads was
// changed, and both copies of prices.json were rebuilt.
//
// So it is now an ASSERTION, not a note. A NOTE is the right shape for a divergence
// nobody can fix from here; it is the wrong shape once they agree, because a note is
// invisible in a green run (DESIGN §5 — the runner swallows a suite's stdout) and the
// guide and the bill would silently part company again.
console.log('\n=== SHEET vs data/prices.json ===');
{
  ...
  if (pjGround === SHEET.oc_each) {
    ...
    fail(`data/prices.json OC:ground_inurnment = ${money(pjGround)}, the 2026-07-31 sheet ` +
         `prices every urn-garden O&C at ${money(SHEET.oc_each)}. The guide prints the sheet's ` +
         `figure, so the tool would bill something the guide does not say. prices.json is ` +
         `GENERATED — change the source (the qOCGround label build-prices.py scrapes) and ` +
         `rerun the map's build-prices.py, then npm run sync-prices. Never hand-edit it.`);
  }
  // The sheet is SILENT on boulder inurnment, so prices.json governs there and there is
  // nothing to reconcile — recorded so the silence reads as checked, not overlooked.
  note(`the sheet has no boulder column; OC:boulder_inurnment stays at ` +
       `${money(prices.current.fees['OC:boulder_inurnment'])} from data/prices.json.`);
