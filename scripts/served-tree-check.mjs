// Guard against the phantom-result failure this repo hit on 2026-07-26.
//
...
//
// The old check only asked whether *something* answered on 3737. Checking that the server
// returns byte-for-byte what is on disk here is what actually closes it.
import fs from 'fs';
...
export async function assertServesThisTree(base, rootDir, what = 'this run') {
  ...
  throw new Error(
    ...
    `worktree or from main will answer here and produce results for the WRONG code.\n` +
    `A baseline capture in that state reports "identical" and means nothing.\n\n` +
    `Fix: stop the other server, or start this tree's own on a free port\n` +
