...
### 2026-08-10 — Sprint-22: Travel Plan by Inman + Bronze Marker guides
Pushed (this entry written at push time). Two new family guides.
- `inman-travel-plan-guide.html` + `pdf-assets/Travel Plan by Inman.pdf` (4pp, cap 4) + Getting Started card. Enrollment link (MAMOWEST1) on page and PDF per Martice. Prices from the live enrollment page ($499/$974, 3/12-mo plans). Region-price graphic generated by `scripts/build_inman_region_map.mjs` (Inman source map PDF is off-brand and prints below the type floor; never ship it).
- `bronze-markers-guide.html` + `pdf-assets/Bronze Markers.pdf` (5pp, cap 5) + Markers & Memorials card. All figures tax-included (10.4% folded in per Martice 2026-08-10), verified against CEMETERY MERCH & SERVICES PRICE LIST EFF-03.01.2026. Half ledger = 24x30 on 28x34. Vase cores include only the fitting; vases sold separately. Images: `bronze-images/` (Matthews x4, Coldspring x1, credited).
- Print-system scars: data-pdf="summary" on a wrapper hides the wrapper own h3/p (annotate the paragraph); summary-block kickers need hand numbers; break-after:avoid loses to unbreakable next blocks (use keep-together).
